...
export const endpoints = {
  ...
  getMenuItemsByTypeAndCategory: (type, category_id) => {
    return type === 'all' ? `/menu/item?category_id=${category_id}` : `/menu/item?&type=${type}&category_id=${category_id}`},

  toggleMenuItemStock: (id, action = 'toggle-stock') => `/menu/item/${id}/${action}`,
  ...
  toggleCouponStatus: (id, action = 'toggle-status') => `/coupon/${action}/${id}`,

  // Order Related
  getOrder: '/order',
  getOrderByStatus: (order_status) => `/order?order_status=${order_status}`,
  updateOrderStatus: (id, action = 'status') => `/order/${id}/${action}`,
  updateOrderPaymentStatus: (id, action = 'payment-status') => `/order/${id}/${action}`,

  //Promoc Related
  getPromoCode: '/promocode',
  addPromoCode: '/promocode',
  togglePromodCodeStatus:(id, action = 'toggle-status') => `/promocode/${action}/${id}`,
