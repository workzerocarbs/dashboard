...
export const createAddOn = async (formData, groupName, quantity) => {
  ...
  try {
    const response = await axios.post(
      `${base_url}${endpoints.createAddOn}`,
      data,
      {
        headers: {
    ...
    if (error.response) {
      ...
      throw new Error(
        error.response.data.message || "Failed to create add-on group"
      );
...
export const addAddon = async (itemId, checkedAddons) => {
  ...
  try {
    const response = await axios.post(
      `${base_url}${endpoints.addAddOnToItem}`,
      data,
      {
        headers: {
          "Content-Type": "multipart/form-data",
...
export const removeAddOn = async (itemId, unselectedAddOns) => {
  ...
  try {
    const response = await axios.post(
      `${base_url}${endpoints.removeAddonFromItem}`,
      data,
      {
        headers: {
          "Content-Type": "multipart/form-data",
...
// Update AddOn Group
export const updateAddOnGroup = async (id, formData) => {
  try {
    const response = await axios.post( `${base_url}${endpoints.updateAddOn(id)}`, formData, {
      headers: {
        "Content-Type": "multipart/form-data",
        Authorization: `Bearer ${token}`,
      },
...
// delete AddOn Group
export const deleteAddOnItem = async (id) => {
  try {
    const response = await axios.delete( `${base_url}${endpoints.deleteAddOnById(id)}`, {
      headers: {
        Authorization: `Bearer ${token}`,
      },
    });
