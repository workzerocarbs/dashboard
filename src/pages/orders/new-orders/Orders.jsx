      ...
      prevOrders.map((order) => {
        if (order.id === id) {
          ...
          return {
            ...order,
            timer: newTimer,
            initialTimer: order.initialTimer + 60,
          };
        }
        return order;
      })
      ...
      prevOrders.map((order) => {
        if (order.id === id) {
          ...
          return {
            ...order,
            timer: newTimer,
            initialTimer: order.initialTimer + (additionalTime[id] || 0),
            isPaused: false,
          };
        }
        return order;
        ...
        <div className="border-0">
          <div className="card-body">
            <div className="order-tabs">
              ...
              <Button
                className={activeTab === "Picked Up" ? "active" : ""}
                onClick={() => handleTabClick("Picked Up")}
              >
                Picked Up{" "}
                <span>
                  (
                ...
                <li key={order.id} className="order-details-body">
                  ...
                  <div className="rightBox">
                          ...
                          <div className="item-info">
                            <div className="item-box">
                              <div className="type-image">
                                <img
                                  src={
                                    item.type === "veg" ? vegImage : nonVegImage
                                  }
                    ...
                    <div className="additional-info">
                      ...
                      <p>
                        Order Instructions: <span>{order.request}</span>
                      </p>
                    </div>
                    <hr />
                    ...
                    {order.status === "New" && (
                      <>
                        ...
                        <Button
                          className="btn bg-primary w-100 text-white mt-2"
            ...
            <Button className='confirm-btn' onClick={() => handleConfirm(order.id)}>Confirm</Button>
                    ...
                    {order.status === "Preparing" && (
                      <div>
                        <div
                          className={`progress-text progress-bar-container ${
                            order.timer === 0
                              ? "blinking-background"
                        ...
                        >
                          <div
                            ...
                            style={{
                              width: `${
                                ((order.initialTimer - order.timer) /
                                  order.initialTimer) *
                                100
                              }%`,
                          ...
                          >
                            {order.timer === 0
                              ? `Mark as Ready`
                              : `Preparing Order (${formatTime(order.timer)})`}
                          </p>
                          ...
                          <div className="more-time-btn">
                            <div className="add-time-btn">
                              ...
                              <div className="time-slabs">
                                <div
                                  className={`slab  ${
                                    activeSlabs[order.id] === 5 ? "active" : ""
                                  }`}
                                  onClick={() => handleAddTime(order.id, 5)}
                                >
                                  +5 min
                                </div>
                                <div
                                  className={`slab ${
                                    activeSlabs[order.id] === 10 ? "active" : ""
                                  }`}
                                  onClick={() => handleAddTime(order.id, 10)}
                                >
                                  +10 min
                                </div>
                                <div
                                  className={`slab ${
                                    activeSlabs[order.id] === 15 ? "active" : ""
                                  }`}
                                  onClick={() => handleAddTime(order.id, 15)}
                                >
                                  +15 min
                                </div>
                                <div
                                  className={`slab ${
                                    activeSlabs[order.id] === 20 ? "active" : ""
                                  }`}
                                  onClick={() => handleAddTime(order.id, 20)}
                                >
                                  +20 min
                                </div>
                                <Button
                                  className="slab-confirm-btn "
                                  onClick={() => handleOk(order.id)}
                                >
                                  Confirm
                                </Button>
                              </div>
