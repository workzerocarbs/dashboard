...
const Sidebar = ({ isSidebarOpen, closeSidebar }) => {
    ...
    const [openSubmenu, setOpenSubmenu] = useState(initialStates.tab !== null ? initialStates.tab : null);
    const [activeSubmenuItems, setActiveSubmenuItems] = useState({ [initialStates.tab]: initialStates.submenu });
        ...
        if (hasSubmenu) {
            if (openSubmenu === index) {
                ...
                if (liRefs.current[index]) {
                    ...
                    const sidebarTop = liRefs.current[0].parentElement.getBoundingClientRect().top;
        ...
        <div className={`sidebar ${isSidebarOpen ? 'open' : 'closed'}`}>
            <ul>
                            ...
                            <ul className="submenu">
                                <li>
                                    <Link to="/menu/category"
                                        className={`w-100 ${activeSubmenuItems[1] === 1 ? 'active-submenu' : ''}`}
                                        onClick={() => handleSubmenuClick(1, 1)}>Categories List</Link></li>
                                <li>
                                    <Link to="/menu/item"
                                        className={`w-100 ${activeSubmenuItems[1] === 2 ? 'active-submenu' : ''}`}
                                        onClick={() => handleSubmenuClick(1, 2)}>Add New</Link></li>
                                <li>
                                    <Link to="/menu/view-menu"
                                        className={`w-100 ${activeSubmenuItems[1] === 3 ? 'active-submenu' : ''}`}
                                        onClick={() => handleSubmenuClick(1, 3)}>View Menu</Link></li>
                            </ul>
                        </div>
                    ) : (
                        openSubmenu === 1 && (
                            <div
                                className="submenu-popup" onMouseLeave={handleMouseLeave}
                ...
                <li ref={(el) => liRefs.current[2] = el}>
                    ...
                    {isSidebarOpen ? (
                        <div className={`submenuWrapper ${openSubmenu === 2 ? 'colapse' : 'colapsed'}`}>
                            <ul className="submenu">
                                <li>
                                    <Link to="/orders/new-orders"
                                        className={`w-100 ${activeSubmenuItems[2] === 1 ? 'active-submenu' : ''}`}
                                        onClick={() => handleSubmenuClick(2, 1)}>New Orders</Link></li>
                                <li>
                                    <Link to="/orders/order-history"
                                        className={`w-100 ${activeSubmenuItems[2] === 2 ? 'active-submenu' : ''}`}
                                        onClick={() => handleSubmenuClick(2, 2)}>Order History</Link></li>
                            </ul>
                        </div>
                    ) : (
                        openSubmenu === 2 && (
                ...
                <li ref={(el) => liRefs.current[3] = el}>
                    ...
                    {isSidebarOpen ? (
                        <div className={`submenuWrapper ${openSubmenu === 3 ? 'colapse' : 'colapsed'}`}>
                            <ul className="submenu">
                                <li>
                                    <Link to="/expenses/view-expenses"
                                        className={`w-100 ${activeSubmenuItems[3] === 1 ? 'active-submenu' : ''}`}
                                        onClick={() => handleSubmenuClick(3, 1)}>View Expenses</Link></li>
                                <li>
                                    <Link to="/expenses/add-new"
                                        className={`w-100 ${activeSubmenuItems[3] === 2 ? 'active-submenu' : ''}`}
                                        onClick={() => handleSubmenuClick(3, 2)}>Add New</Link></li>
                            </ul>
                        </div>
                    ) : (
                        openSubmenu === 3 && (
